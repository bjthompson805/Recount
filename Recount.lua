...
local _, ns = ... -- WoW per-addon namespace; holds ns.Locales / ns.NewLocale (see locales/Recount-LocaleCore.lua)
...
-- UI Language Override (see locales/Recount-LocaleCore.lua). RecountLanguage is a
-- plain SavedVariable, so it is readable here AT LOAD -- before this file and the
-- GUI capture L[...] into their own tables (the mode list, option tables, etc.).
-- "auto" or unset leaves AceLocale's client-locale strings untouched. Any other
-- value overlays that locale's strings onto the shared L table, so every later
-- L[...] read uses the chosen language. In-game item / spell / unit names come
-- from the client and are unaffected. Changing it in Settings writes
-- RecountLanguage and takes effect on the next /reload.
do
	local override = _G.RecountLanguage
	if override and override ~= "auto" and ns.Locales and ns.Locales[override] then
		for k, v in pairs(ns.Locales[override]) do
			L[k] = v
		end
	end
end
...
Recount.Version = tonumber(string.sub("$Revision: 1604 $", 12, -3))
...
-- 12.0 (Midnight) introduced C_RestrictedActions. Calling RegisterEvent for
-- COMBAT_LOG_EVENT_UNFILTERED while any of {Combat, Encounter, ChallengeMode,
-- PvPMatch} is active fires ADDON_ACTION_FORBIDDEN. We can't catch that with
-- pcall (it's async from Blizzard's side); the only avoidance is to not make
-- the call when restrictions are active. Pre-12.0 retail and all Classic
...
Recount.events:SetScript("OnEvent", function(_self, event, ...)
...
-- TRIMMED TO WHAT THIS FILE READS, same as the sibling copy in Tracker.lua.
--
-- NOTE THIS IS THE SECOND COPY of the same upstream table -- Tracker.lua:71
-- carries the other, with its own `LIB_FILTER_*` set built from it. They are
-- file-locals so neither can see the other, and the two had already diverged:
-- this one folded REACTION_FRIENDLY into LIB_FILTER_ME and Tracker.lua's did
-- not. Left as two copies rather than promoted to a shared module, because the
-- masks each file actually uses are disjoint and merging them is a wider change
-- than a lint sweep should make; recorded here so the duplication is visible to
-- whoever does make it.
...
-- No LIB_FILTER_PARTY / _RAID / _GROUP here: they existed only to build each
-- other and nothing in this file read the result. Tracker.lua keeps the ones the
-- combat-log path actually tests against.
...
local function deepcopy(object)
	local lookup_table = {}
	-- `node`, not `object`: the inner function shadowed the outer argument, so the
	-- two names in one function meant different things at different depths.
	local function _copy(node)
		if type(node) ~= "table" then
			return node
		elseif lookup_table[node] then
			return lookup_table[node]
		end
		local new_table = {}
		lookup_table[node] = new_table
		for index, value in pairs(node) do
			new_table[_copy(index)] = _copy(value)
		end
		return setmetatable(new_table, getmetatable(node))
	end
	return _copy(object)
end
...
Recount.consoleOptions = {
	...
	args = {
		...
		uiLanguage = {
			...
			desc = L["Choose the language for Recount's own text. Game item, spell, and unit names still use your client's language."],
			-- Native-script labels, intentionally not localized, so users recognise
			-- their language regardless of the current UI language. Scripts that
			-- WoW's default Western fonts can't render (CJK, Thai) carry an English
			-- hint in parentheses so they're still identifiable as boxes.
			values = {
				auto  = "Auto",
				enUS  = "English",
				deDE  = "Deutsch",
				...
				koKR  = "한국어 (Korean)",
				zhCN  = "简体中文 (Chinese, CN)",
				zhTW  = "繁體中文 (Chinese, TW)",
				thTH  = "Thai", -- WoW's default fonts lack Thai glyphs; native "ไทย" would render as boxes
				filPH = "Filipino",
			},
			sorting = { "auto", "enUS", "deDE", "esES", "esMX", "frFR", "itIT", "nlNL", "ptBR", "ruRU", "koKR", "zhCN", "zhTW", "thTH", "filPH" },
			get = function() return _G.RecountLanguage or "auto" end,
			set = function(_, val)
				_G.RecountLanguage = val
				Recount:Print(L["Recount language updated - type /reload to apply it everywhere."])
			end,
		},
		[L["sync"]] = {
			order = 21,
		...
		minimap = {
			...
			type = "toggle",
			name = "Minimap Button",
			desc = "Show or hide the Recount minimap button. The button's position around the minimap ring is saved per profile.",
			get = function(_info)
				return not (Recount.db.profile.minimapButton and Recount.db.profile.minimapButton.hide)
			end,
			set = function(_info, v)
				Recount:ToggleMinimapButton(v)
			end,
		},
	}
}
...
function Recount:NrCombatants()
	local counter = 0
	for _ in pairs(dbCombatants) do
		counter = counter + 1
	end
	return counter
end
...
function Recount:ResetDataUnsafe()
	...
	for k in pairs(dbCombatants) do
		Recount:DeleteGuardianOwnerByGUID(dbCombatants[k])
		dbCombatants[k] = nil
	end
	for k in pairs(Recount.db2.CombatTimes) do
	...
	-- Midnight keeps the numbers in Blizzard's C_DamageMeter session, not in
	-- dbCombatants, so this has to run before the refresh below or the bars are
	-- re-rendered from the data we just failed to clear.
	if Recount.ResetData_Midnight then
		Recount:ResetData_Midnight()
	end
	...
	for _, v in pairs(dbCombatants) do
		v.LastFightIn = 0
	end
...
function Recount:ResetFightData(data)
	-- A nil `data` is a no-op: the old code built an empty table and dropped it on
	-- return, which reads as "start a fresh record" and does nothing of the kind.
	if not data then
		return
	else
...
function Recount:FindPetUnitFromFlags(unitFlags, unitGUID)
	...
	-- Check for my pet
	-- The affiliation test has to be masked against unitFlags. Without the first
	-- argument, bit.band(MINE) just returns MINE -- non-zero for every pet -- so
	-- EVERY pet resolved to "pet", the player's own pet token, and unit lookups
	-- for other players' pets pointed at the wrong unit.
...
function Recount:ScanGUIDTooltip(nameGUID)
	...
	-- The two `= nil` else-branches are gone: this is a debug dump that prints each
	-- line and keeps nothing, so clearing a variable nothing reads afterwards did
	-- nothing. Both are now scoped to the iteration that uses them.
	for idx = 1, RecountTempTooltip:NumLines() do
		local ttextLeft = _G[tooltipName.."TextLeft"..idx]
		if ttextLeft then
			local textLeft = ttextLeft:GetText()
			if textLeft then
				Recount:Print("left"..idx..": "..textLeft)
			end
		end
		local ttextRight = _G[tooltipName.."TextRight"..idx]
		if ttextRight then
			local textRight = ttextRight:GetText()
			if textRight then
...
-- No lastSummonOwnerName: it was written on every summon and read nowhere, so it
-- was a record of the last owner that nothing consulted. lastSummonOwnerGUID is
-- the one the inheritance check below actually compares against.
...
-- Recount:PartyPetOwnerFromGUID was here. It had NO CALLER anywhere in the addon
-- and its only statement was a commented-out Recount:SetOwner, so the lookup it
-- performed was discarded -- it could not have had an effect even if something
-- had called it. Deleted rather than left as an entry point that does nothing;
-- Recount:SetOwner below is the live path and is reached from AddPetCombatant.

function Recount:SetOwner(who, petName, owner, ownerGUID, ownerFlags)
	if owner then
		-- Record the owner ON THE PET as well as the pet on the owner. Non-pets
		-- get `Owner = false` in AddCombatant, and three separate call sites
		-- read `.Owner` expecting the owner's NAME for a pet -- the lazy-sync
		-- push, DeleteCombatant's pet cleanup, and the "Merge Pets" filter's
		-- owner inheritance. Nothing ever set it, so all three were dead.
		if who then
			who.Owner = owner
		end
		...
		for _, k in ipairs(dbCombatants[owner].Pet) do -- Prevent multi-pet registration
			if k == petName then
				return
			end
		end
			...
			if filters.Data[ctype] and filters.TimeData[ctype] and v.TimeLast and v.TimeLast["OVERALL"] and v.TimeLast["OVERALL"] >= TimeCheck then -- Elsia: Added global collection switch
				...
				for k, v2 in pairs(v.TimeWindows) do
					...
					if v.TimeLast and v.TimeLast[k] and v.TimeLast[k] >= TimeCheck then
						...
						-- `slot`, not `k`: the enclosing `k` is the TimeWindows key
						-- and is read two lines up at `v.TimeLast[k]`, so reusing the
						-- name here shadowed a live variable with a sample index.
						for slot, v3 in ipairs(v2) do
							Temp = (FilterSize - slot) + Recount.TimeStep
...
function Recount:OnInitialize()
	...
	-- Second return is the Blizzard category identifier: a string on older clients,
	-- a numeric category ID on clients with C_SettingsUtil (which rejects strings).
	local _, blizCategory = AceConfigDialog:AddToBlizOptions("Recount Blizz", "Recount")
	Recount.BlizOptionsCategory = blizCategory
	...
	-- Isolate each init step so a failure in one doesn't prevent SetupMainWindow
	-- from running (which is the only path that binds MainWindow.GetData and
	-- makes the bars render). The chat warning surfaces the failure to users
	-- so they can report it instead of seeing a silently-empty meter.
	local function safeInit(name, fn)
		local ok, err = pcall(fn)
		if not ok then
			DEFAULT_CHAT_FRAME:AddMessage("|cffff8800Recount init warning:|r " .. name .. " failed: " .. tostring(err))
		end
	end
	safeInit("CreateMainWindow",    function() Recount:CreateMainWindow()    end)
	safeInit("CreateDetailWindow",  function() Recount:CreateDetailWindow()  end)
	safeInit("CreateGraphWindow",   function() Recount:CreateGraphWindow()   end)
	safeInit("CreateFilterWeights", function() Recount:CreateFilterWeights() end)
	safeInit("InitOrder",           function() Recount:InitOrder()           end)
	safeInit("SetupMainWindow",     function() Recount:SetupMainWindow()     end)
	...
	-- Enable VersionCheck-1.0 integration
	do
		local VC = LibStub:GetLibrary("VersionCheck-1.0", true)
		if VC and VC.Enable then
			local hostAddon = {
				GetName = function() return "Recount" end,
				Version = (C_AddOns and C_AddOns.GetAddOnMetadata("Recount", "Version")) or GetAddOnMetadata("Recount", "Version") or "Recount-v1.18.8",
			}
			VC:Enable(hostAddon)
		end
	end
	Recount:InitMinimapButton()
	Recount:InitTitanPanel()
end

-- Patch 12.0 (Midnight) removed addon access to COMBAT_LOG_EVENT_UNFILTERED.
-- Calling Frame:RegisterEvent("CLEU") fires ADDON_ACTION_FORBIDDEN in every
-- context, including login. C_RestrictedActions.IsAddOnRestrictionActive is
-- not a useful gate here (it reports "no restrictions" at login while the
...
function Recount:RegisterCombatLogEvent()
	if WOW_RETAIL_MIDNIGHT then
		if Recount.RegisterCombatLogEvent_Midnight then
			return Recount:RegisterCombatLogEvent_Midnight()
		end
		return false
	end
...
function Recount:OnEnable()
	...
	-- Parser Events
	Recount:RegisterCombatLogEvent()
...
function Recount:PetBattleUpdate()
	if Recount.db.profile.HidePetBattle and WOW_PANDA_CLASSIC and C_PetBattles.IsInBattle() and Recount.MainWindow:IsShown() then
		Recount.MainWindow:Hide()
...
function Recount:OnDisable()
	...
	-- The client events live on Recount.events, a plain frame. Recount embeds
	-- AceConsole / AceComm / AceTimer but NOT AceEvent, so the addon object has
	-- no UnregisterAllEvents of its own -- calling it here threw.
	Recount.events:UnregisterAllEvents()
end
