...
function Recount:MergedPetDamageDPS(data, fight)
	...
	if Recount.db.profile.MergePets and data.Pet then
		-- The VALUE is the pet's combatant name; the key is unused. Named `k` for
		-- the value here since forever, which reads backwards -- left alone rather
		-- than renamed, because every line below indexes `dbCombatants[k]`.
		for _, k in pairs(data.Pet) do
...
-- The breakdown tables behind a MERGED bar, with the pets' own rows folded in.
--
-- MergedPetDamageDPS/MergedPetHealingDPS above sum the pet's TOTAL into the
-- owner's bar and stop there, so with "Merge Pets w/ Owners" on the bar read
-- owner+pet while the detail window under it listed the owner's abilities only
-- and summed to owner. The two numbers disagreed with nothing on screen saying
-- why, which reads as "the pet is not being recorded" -- it always was, under its
-- own combatant. Reported from a live hunter whose detail window showed seven of
-- his own abilities and no pet row at all.
--
-- Pet rows are keyed `Ability (Petname)` rather than summed into the owner's row:
-- the pet's Claw is not the owner's ability, and merging the two would also mean
-- merging their `Details` sub-tables, which is what the drill-down reads.
--
-- The owner's table is passed straight back when there is nothing to merge, so
-- the common case allocates nothing; the copy is shallow and every value is
-- read-only to the detail view, so the stored tables are never touched.
local function mergedPetBreakdown(data, fight, key)
	local ownerTable = data.Fights[fight][key]
	if not (Recount.db.profile.MergePets and data.Pet) then
		return ownerTable
	end

	local merged
	for _, petName in pairs(data.Pet) do
		local pet = dbCombatants[petName]
		local petTable = pet and pet.Fights and pet.Fights[fight] and pet.Fights[fight][key]
		if petTable and next(petTable) then
			if not merged then
				merged = {}
				for k, v in pairs(ownerTable or {}) do
					merged[k] = v
				end
			end
			-- `Name` is the bare pet name; the combatant key carries " <Owner>".
			local label = pet.Name or petName
...
function DataModes:DamageReturner(data, num)
	...
	local set = Recount.db.profile.CurDataSet
	return damage, {{mergedPetBreakdown(data, set, "Attacks"), L["'s Hostile Attacks"], DetailTitles.Attacks}, {mergedPetBreakdown(data, set, "DamagedWho"), " "..L["Damaged Who"], DetailTitles.DamagedWho}, {data.Fights[set].PartialResist, L["'s Partial Resists"], DetailTitles.Resisted}, {data.Fights[set].TimeDamaging, L["'s Time Spent Attacking"], DetailTitles.DamageTime}}
end
...
function DataModes:DPSReturner(data, num)
	...
	local set = Recount.db.profile.CurDataSet
	return dps, {{mergedPetBreakdown(data, set, "Attacks"), L["'s Hostile Attacks"], DetailTitles.Attacks}, {mergedPetBreakdown(data, set, "DamagedWho"), " "..L["Damaged Who"], DetailTitles.DamagedWho}, {data.Fights[set].PartialResist, L["'s Partial Resists"], DetailTitles.Resisted}, {data.Fights[set].TimeDamaging, L["'s Time Spent Attacking"], DetailTitles.DamageTime}}
end
...
function DataModes:HealingReturner(data, num)
	...
	local set = Recount.db.profile.CurDataSet
	return healing, {{mergedPetBreakdown(data, set, "Heals"), L["'s Effective Healing"], DetailTitles.Heals}, {mergedPetBreakdown(data, set, "HealedWho"), " "..L["Healed Who"], DetailTitles.HealedWho}, {data.Fights[set].OverHeals, L["'s Overhealing"], DetailTitles.OverHeals}, {data.Fights[set].TimeHealing, L["'s Time Spent Healing"], DetailTitles.HealTime}}
end
...
-- An Ace2 table-recycling pair (`new` / `del` over a weak cache) lived here and
-- had no caller anywhere in the file or the addon. Recount recycles through
-- Recount:GetTable() / Recount:FreeTable() instead, which is what this file
-- actually calls, so the pair was a second unused implementation of the same
-- idea. Deleted rather than kept "in case".
...
function Recount:AddSortedTooltipData(title, data, num)
	...
	local percent
	-- `rank`, not `i`: the enclosing `i` above is the COUNT of rows collected and
	-- is still read at the `num > i` clamp, so reusing the name here shadowed a
	-- live variable with an unrelated one.
	for rank = 1, num do
		if SortedData[rank] then
			if total ~= 0 then
				percent = math_floor(100 * SortedData[rank][2] / total)
			else
				percent = 100
			end
...
function TooltipFuncs:Damage(name, data)
	if data then
		...
		GameTooltip:AddLine("<Shift+Click for DPS Graph>", 0, 0.9, 0)
	end
end
...
function TooltipFuncs:DeathCounts(name, _data)
	GameTooltip:ClearLines()
	GameTooltip:AddLine(name)
	--GameTooltip:Hide()
end
...
function DataModes:ThreatReturner(data, num)
	if not data or not data.Fights[Recount.db.profile.CurDataSet] then
		return 0
	end
	local peak = data.Fights[Recount.db.profile.CurDataSet].ThreatPeak or 0
	if num == 1 then
		return peak
	end
	return peak
...
function TooltipFuncs:Threat(name, data)
	GameTooltip:ClearLines()
	GameTooltip:AddLine(name)
	if data and data.Fights and data.Fights[Recount.db.profile.CurDataSet] then
		local peak = data.Fights[Recount.db.profile.CurDataSet].ThreatPeak or 0
		GameTooltip:AddDoubleLine("Peak Threat:", string.format("%.1fk", peak), nil, nil, nil, 1, 1, 1)
		GameTooltip:AddLine("<Shift+Click for Threat Graph>", 0, 0.9, 0)
	end
end
...
local MainWindowModes = {
	...
	{L["Threat"], DataModes.ThreatReturner, TooltipFuncs.Threat, nil, {"Threat", "'s Threat (k)"}, nil, "Threat"},
}
...
function Recount:SetupMainWindow()
	if Recount.LoadMainWindowData then -- If we add modes early no need to inform the main window, it will get set up later.
		...
		if Recount.db.profile.Modules.Threat then
			local found
			for k in pairs(MainWindowModes) do
				if MainWindowModes[k][1] == L["Threat"] then
					found = true
					break
				end
			end
			if not found then
				tinsert(MainWindowModes, {L["Threat"], DataModes.ThreatReturner, TooltipFuncs.Threat, nil, {"Threat", "'s Threat (k)"}, nil, "Threat"})
			end
		else
			for k in pairs(MainWindowModes) do
				if MainWindowModes[k][1] == L["Threat"] then
