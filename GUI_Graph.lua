...
-- `LibGraph`, not `Graph`: five functions in this file open with
-- `local Graph = Recount.GraphWindow.LineGraph`, which is the graph FRAME, and
-- every one of them shadowed the LIBRARY under the same name. The library is
-- reached exactly once, to build that frame; renaming it here is a two-line
-- change where renaming the five frame locals would have meant ~30 uses.
local LibGraph = LibStub:GetLibrary("LibGraph-2.0")
...
function me:RefreshGraph()
	...
	local Length, MaxLength, Filtered, MaxAmount, DataMax, Temp
	...
	MaxLength = 0
	MaxAmount = 0
	-- No MinAmount: it was zeroed here and read nowhere, and since it was dropped
	-- from the `local` list above this line was writing a GLOBAL of that name.
	...
	if not Recount.TimeRangeSet then
		...
		if Recount.GraphWindow.Data then
			for _, v in pairs(Recount.GraphWindow.Data) do
				if v[1][1] then
...
function Recount:GraphRefreshData()
	...
	if Recount.GraphWindow.Data then
		for k in pairs(Recount.GraphWindow.Data) do
...
function Recount:CreateGraphWindow()
	...
	theFrame:SetScript("OnMouseUp", function(this)
		if (this.isMoving) then
			local xOfs, yOfs = select(4, this:GetPoint(1))
	...
	theFrame.LineGraph = LibGraph:CreateGraphLine("Recount_GraphWindow_LineGraph", theFrame, "TOPLEFT", "TOPLEFT", 1, -32, 400, 418)
	...
	theFrame.GraphBackground:SetScript("OnUpdate", function(this)
		...
		-- IsMouseOver() rather than the MouseIsOver global: retail 12.0 dropped
		-- that global, and this file cached it as an upvalue at load, so it would
		-- be nil for the whole session. See the note in LibGraph-2.0.lua's
		-- PieChart_OnUpdate -- the global was only a forwarder to this method.
		if Recount.TimeRangeUpper and (GraphWindow.GraphBackground:IsMouseOver() or Recount.GraphWindow.Dragging) then
			...
			if GraphWindow.GraphBackground:IsMouseOver() then
	...
	-- "RIGHT", not "Right". Anchor points are an enum and the client REJECTS a
	-- mis-cased one, so this line raised "invalid anchor point" every time
	-- CreateGraphWindow ran -- which the init guard swallowed into a chat
	-- warning, leaving `theFrame.Rows = {}` below unreached and the graph
	-- window's legend permanently empty. The offline env used to store SetPoint's
	-- arguments without validating them, which is why no spec could see it.
	Labels.Enabled = Labels:CreateFontString(nil, "OVERLAY", "GameFontNormal")
	Labels.Enabled:SetPoint("RIGHT", Labels, "RIGHT", -2, 0)
	Labels.Enabled:SetText(L["Enabled"])
	...
	theFrame.ScrollBar1:SetScript("OnVerticalScroll", function(scrollFrame, offset)
		FauxScrollFrame_OnVerticalScroll(scrollFrame, offset, 20, Recount.GraphRefreshData)
	end)
	...
	-- Reassigned, not re-declared: two label frames are built in this function
	-- and the second opened with its own `local Labels`, shadowing the first.
	Labels = theFrame.TimeLabels
	...
	theFrame.ScrollBar2:SetScript("OnVerticalScroll", function(scrollFrame, offset)
		FauxScrollFrame_OnVerticalScroll(scrollFrame, offset, 20, Recount.GraphRefreshCombat)
	end)
