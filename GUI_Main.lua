...
-- Patch 12.0 (Midnight) and newer: addons cannot read COMBAT_LOG_EVENT_UNFILTERED
-- and combat values arrive as "secret" values. The Midnight bar renderer lives in
-- Tracker_Midnight.lua and is the only path allowed to drive bars on these clients.
-- On every other client this flag is false and RefreshMainWindow runs unchanged.
...
function Recount.CommaNumber(value)
	...
	local left, num = string_match(value, "^([^%d]*%d)(%d+)(.-)$")
...
-- `fontString`, not `string`: the parameter shadowed the `string` library cached
-- at the top of this file. Nothing in the body called it, so this was latent.
function me:SetFontSize(fontString, size)
...
function Recount:OpenBarDropDown(myframe)
	...
	--
	-- The declaration used to carry `= barmenuframe and barmenuframe:Release()`
	-- with a "release if already shown" comment. It NEVER RELEASED ANYTHING: the
	-- local is fresh on every call, so the guard was always nil. The initialiser
	-- is gone; the variable itself is real and is assigned and read below.
	-- Making the release work means promoting the handle to a file-local, which
	-- changes when the dropdown is torn down -- a behaviour change, not a lint
	-- fix, so it is left alone. The same dead initialiser sat above the fight and
	-- mode dropdowns and is gone from both.
	-- Declared before `baropts` because the menu callbacks inside it close over
	-- this variable and run after the assignment further down.
	local barmenuframe
	local baropts = {
		type = 'group',
		args = {
	...
	barmenuframe = LD:OpenAce3Menu(baropts)
...
function Recount:SetupBar(row)
	...
	-- The old form declared `BarTexture` and then tested `if not BarTexture` before
	-- anything had assigned it, so that branch could only ever be taken. Same
	-- shape as GUI_Config's BarTextureDropDown_Initialize; the intent is the
	-- profile's texture with a BantoBar fallback.
	local BarTexture = SM:Fetch("statusbar", Recount.db.profile.BarTexture or "BantoBar")
...
function me:CreateRow(num)
	...
	if num ~= 0 then
		-- `bar`, not `self`: these run on the row button, and `me:CreateRow` already
		-- has a `self` of its own (the GUI module table).
		row:SetScript("OnClick", function(bar, button)
			if button == "RightButton" then
				Recount:OpenBarDropDown(bar)
				--[[Recount:BarDropDownOpen(bar)
				CloseDropDownMenus(1)
				ToggleDropDownMenu(1, nil, Recount_BarDropDownMenu)]]
			elseif type(bar.clickFunc) == "function" and bar.clickData then
				bar:clickFunc(bar.clickData)
			end
		end)
		row:SetScript("OnEnter", function(bar)
			GameTooltip:SetOwner(bar, "ANCHOR_TOPRIGHT")
			Recount.MainWindow:TooltipFunc(bar.Name, bar.TooltipData)
			GameTooltip:Show()
		end)
...
function Recount:CreateMainWindow()
	...
	-- `win` / `grip` / `title` below, not `self`: the enclosing window-builder has
	-- its own `self` and every one of these scripts was shadowing it.
	theFrame:SetScript("OnSizeChanged", function(win)
		if (win.isResizing) then
			Recount:ResizeMainWindow()
			Recount.db.profile.MainWindowHeight = win:GetHeight()
			Recount.db.profile.MainWindowWidth = win:GetWidth()
		end
	end)
	...
	theFrame.TitleClick:SetScript("OnMouseDown", function(title, button)
		if button == "RightButton" then
			Recount:OpenModeDropDown(title)
			--[[Recount:ModeDropDownOpen(title)
	...
	theFrame.TitleClick:SetScript("OnMouseUp", function(title)
		local parent = title:GetParent()
	...
	theFrame.TitleClick:SetScript("OnMouseWheel", function (_title, delta)
	...
	theFrame.TitleClick:SetScript("OnEnter", function(title)
		GameTooltip:SetOwner(title, "ANCHOR_BOTTOM")
		GameTooltip:AddLine("Right-click to select display mode")
		GameTooltip:AddLine("Alt + Scroll to cycle modes")
		GameTooltip:Show()
	end)
	theFrame.TitleClick:SetScript("OnLeave", function()
		GameTooltip:Hide()
	end)

	theFrame.ScrollBar = CreateFrame("SCROLLFRAME", "Recount_MainWindow_ScrollBar", theFrame, "FauxScrollFrameTemplate")
	theFrame.ScrollBar:SetScript("OnVerticalScroll", function(scrollFrame, offset)
		FauxScrollFrame_OnVerticalScroll(scrollFrame, offset, 20, Recount.RefreshMainWindow)
	end)
	...
	theFrame.DragBottomRight:SetScript("OnMouseDown", function(grip, button)
		if (((not grip:GetParent().isLocked) or (grip:GetParent().isLocked == 0)) and (button == "LeftButton")) then
			grip:GetParent().isResizing = true
			grip:GetParent():StartSizing("BOTTOMRIGHT")
		end
	end) -- Elsia: disallow resizing when locked.
	theFrame.DragBottomRight:SetScript("OnMouseUp", function(grip)
		if grip:GetParent().isResizing == true then
			grip:GetParent():StopMovingOrSizing()
			grip:GetParent():SaveMainWindowPosition()
			grip:GetParent().isResizing = false
		end
	end)
	...
	theFrame.DragBottomLeft:SetScript("OnMouseDown", function(grip, button)
		if (((not grip:GetParent().isLocked) or (grip:GetParent().isLocked == 0)) and (button == "LeftButton")) then
			grip:GetParent().isResizing = true
			grip:GetParent():StartSizing("BOTTOMLEFT")
		end
	end) -- Elsia: disallow resizing when locked.
	theFrame.DragBottomLeft:SetScript("OnMouseUp", function(grip)
		if grip:GetParent().isResizing == true then
			grip:GetParent():StopMovingOrSizing()
			grip:GetParent():SaveMainWindowPosition()
			grip:GetParent().isResizing = false
	...
	theFrame.FileButton:SetScript("OnClick", function(fileButton)
		Recount:OpenFightDropDown(fileButton)
		--[[Recount:FightDropDownOpen(fileButton)
	...
	theFrame.CompareButton = CreateFrame("Button", nil, theFrame)
	theFrame.CompareButton:SetNormalTexture("Interface\\Buttons\\UI-GuildButton-OfficerNote-Up.blp")
	theFrame.CompareButton:SetHighlightTexture("Interface\\Buttons\\UI-Panel-MinimizeButton-Highlight.blp")
	theFrame.CompareButton:SetWidth(16)
	theFrame.CompareButton:SetHeight(16)
	theFrame.CompareButton:SetPoint("RIGHT", theFrame.ReportButton, "LEFT", 0, 0)
	theFrame.CompareButton:SetScript("OnClick", function()
		Recount:OpenCompareWindow()
	end)
	theFrame.CompareButton:SetFrameLevel(theFrame.CompareButton:GetFrameLevel() + 1)
	theFrame.CompareButton:SetScript("OnEnter", function(this)
		GameTooltip:SetOwner(this, "ANCHOR_BOTTOM")
		GameTooltip:SetText("Compare Graph")
		GameTooltip:AddLine("Open a graph window to compare\nDPS / TPS / Healing across players.", 1, 1, 1)
		GameTooltip:Show()
	end)
	theFrame.CompareButton:SetScript("OnLeave", function() GameTooltip:Hide() end)
...
-- A ConvertName table mapping data-set keys to "(Overall)" / "(Current)" /
-- "(Last)" suffixes lived here with no reader. The window title is built from
-- Recount.FightName instead, which the fight dropdown sets directly.
...
function me:ShowGraphWindow(name)
	-- Only pass the series relevant to the current mode so e.g. Shift+Click from
	-- "Damage Done" shows the DPS curve without the TPS series unexpectedly
	-- appearing alongside it.  Modes with no DataComparing key (nil) fall back to
	-- showing the full TimeData dict (original Recount behaviour).
	local DataComparing = Recount.MainWindowData[Recount.db.profile.MainWindowMode][7]
	local timeData = dbCombatants[name].TimeData
	local data
	if DataComparing and timeData and timeData[DataComparing] then
		data = { [DataComparing] = timeData[DataComparing] }
	else
		data = timeData
	end
	Recount:SetGraphData(name, data, Recount.db2.CombatTimes)
...
function Recount:RefreshMainWindow(datarefresh)
	-- Patch 12.0 (Midnight): the normal renderer below does arithmetic on every
	-- value (totals, sort, percent) which is illegal on Blizzard's secret combat
	-- values -- and those values read as secret to Recount's (tainted) execution
	-- even out of combat, so there is no safe "out of combat use the normal
	-- renderer" path. Hand off to the secret-safe display-passthrough renderer.
	if WOW_RETAIL_MIDNIGHT and Recount.RefreshMainWindow_Midnight then
		return Recount:RefreshMainWindow_Midnight(datarefresh)
	end
	...
	-- Self-heal: if init aborted before SetMainWindowMode bound a display mode,
	-- the timer would otherwise no-op forever and bars would never appear until
	-- the user right-clicked the title to pick a mode manually.
	if not MainWindow.GetData then
		if not Recount.MainWindowData then
			return
		end
		Recount:SetMainWindowMode(Recount.db.profile.MainWindowMode or 1)
	end
	...
	-- No ClassColors local: bar colours come from Recount.Colors, not from this
	-- table, and nothing below read it.
...
function Recount:OpenFightDropDown(myframe)
	-- adopted from BulkMail. The dead "release if already shown" initialiser is
	-- gone; see Recount:OpenBarDropDown for why.
	...
	fightmenuframe = LD:OpenAce3Menu(fightopts)
...
function me:CreateFightDropdown(level)
		-- `entry`, not `info`: the file-local `info` at :374 belongs to
		-- CreateBarDropdown, which uses it about forty times. Renaming this one
		-- keeps that function untouched and removes the shadow.
		local entry = {}

		entry.checked = nil
		entry.text = L["Overall Data"]
		if Recount.db.profile.CurDataSet == "OverallData" then
			entry.checked = 1
		end
		entry.func = function()
...
function Recount:OpenModeDropDown(myframe)
	...
	modemenuframe = LD:OpenAce3Menu(modeopts)
...
function Recount:ReportData(amount, loc, loc2)
	...
	if table.maxn(reportTable) > 0 then
		-- The sort is the point; the top value it exposed was assigned to a
		-- `maxValue` that nothing in this report path ever read.
...
-- Registration point for tracker modules that add their own graph series: maps
-- a TimeData key to the label shown in the graph window's legend.
