...
function me:RefreshDeathDetails()
	...
	for i = 1, 17 do
		Row = Recount.DetailWindow.DeathMode.Deaths[i] or {}
...
function me:CountDeathLogLines()
	...
	for i = 1, #Data.Messages do
		...
		if NextLine ~= "" then
			...
			lines = lines + 1
			-- The RESULT is discarded here but the CALL is not dead: WrapFontString
			-- trims the fontstring in place, and the next SetText depends on that
			-- having happened. Assigning it to NextLine only looked meaningful.
			Recount:WrapFontString(Row.Msg, 235)
		end
		...
		if NextLine ~= "" then
			Row.Msg:SetText(" "..NextLine)
			lines = lines + 1
			-- Result discarded, call kept: WrapFontString trims in place.
			Recount:WrapFontString(Row.Msg, 235)
		end
	end
...
function Recount:CreateDetailWindow()
	...
	PieMode.ScrollBar1:SetScript("OnVerticalScroll", function(scrollFrame, offset)
		FauxScrollFrame_OnVerticalScroll(scrollFrame, offset, 12, me.RefreshUpperDetails)
	end)
	...
	-- REASSIGNED, not re-declared. Four label frames are built in this one
	-- function and each opened with `local Labels = ...`, so three of them
	-- shadowed the one before -- harmless as written, but only because every
	-- block finishes with its own frame before the next begins. Dropping the
	-- `local` keeps that reading and removes the shadow; do NOT rename the
	-- variable per block without renaming its ~20 uses with it.
	Labels = PieMode.BotRowLabels
	...
	DeathMode.ScrollBar1:SetScript("OnVerticalScroll", function(scrollFrame, offset)
		FauxScrollFrame_OnVerticalScroll(scrollFrame, offset, 17, me.RefreshDeathDetails)
	end)
	...
	Labels = DeathMode.DeathLogLabels
	...
	DeathMode.ScrollBar2:SetScript("OnVerticalScroll", function(scrollFrame, offset)
		FauxScrollFrame_OnVerticalScroll(scrollFrame, offset, 12, me.RefreshDeathLogDetails)
	end)
...
function Recount:ReportDetail(amount, loc, loc2)
	...
	if Recount.DetailWindow.PieMode:IsVisible() then
		...
		if not Recount.DetailWindow.Locked then
			...
			-- `Count` was read off the label and never used in either report
			-- branch below, unlike Min/Avg/Max which both branches print.
			local Min, Avg, Max
