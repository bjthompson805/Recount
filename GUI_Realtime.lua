...
-- No `local OpacitySliderFrame = OpacitySliderFrame` here, deliberately. That global has ZERO
-- occurrences in the retail client source, so on a Mainline client the capture was nil and
-- Opacity_Change indexed it unconditionally -- a Lua error the moment a retail user moved the
-- opacity slider on a Realtime window's colour dropdown. Alpha now comes from
-- ColorPickerFrame:GetColorAlpha(), which every flavour defines: Classic Era's returns
-- OpacitySliderFrame:GetValue() (Blizzard_FrameXML/Classic/ColorPickerFrame.lua:19-20) and
-- retail's delegates to Content.ColorPicker (Mainline/ColorPickerFrame.lua:105-107). One call,
-- no feature detection, no flavour branch.
...
local info = {}
function Recount.CreateColorDropdown(_self, level)
	if (not level) then
		return
	end
...
function me:CreateRealtimeWindow(who, tracking, ending) -- Elsia: This function creates a new window and stores it. To other ways, either override it's storage or use the other function
	...
	-- Named `swatch` rather than `self`: this file already has a `self` in scope from the enclosing
	-- method, and shadowing it here reads as the outer object while being the colour swatch.
	g:SetScript("OnMouseDown", function(swatch)
		WhichWindow = swatch.Window
	...
	if Window > 0 then
		...
		if Recount.db.profile.RealtimeWindows and Recount.db.profile.RealtimeWindows[curID] then
			...
			f:SavePosition()
				...
				-- The return is deliberately dropped: creating the window IS the effect.
				me:CreateRealtimeWindow(who, tracking, ending)
		end
	end
end
