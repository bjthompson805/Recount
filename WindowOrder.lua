...
function Recount:SetWindowTop(window)
	...
	-- A window with nothing below it sits directly on UIParent. This happens
	-- whenever SetWindowTop runs on a frame that never went through AddWindow,
	-- which its own OnShow handler will do the first time it is shown.
	if window.Below then
		Recount:SetLevel(window, window.Below:GetFrameLevel() + 10)
	else
		Recount:SetLevel(window, UIParent:GetFrameLevel() + 10)
	end
	TopWindow = window
end
