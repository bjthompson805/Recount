...
function Fights:DeleteOverflowFights(newmax)
	...
	-- The fight NAMES and the death log are per-meter, not per-combatant, so
	-- they are trimmed once. Doing it inside the loop above left them untouched
	-- whenever there were no combatants (e.g. lowering the max right after a
	-- reset), and the fight dropdown kept offering names with no data behind them.
	for i = newmax + 1, Recount.db.profile.MaxFights, 1 do
