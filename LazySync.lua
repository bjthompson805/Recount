...
-- Trimmed to what this file reads. NOTE this is the THIRD copy of the upstream
-- flag table in the addon -- Tracker.lua:71 and Recount.lua:96 carry the others,
-- all three as file-locals that cannot see each other.
...
-- No PARTY_GUARDIAN_FLAGS: it was built and never used. Sync bootstraps pets and
-- pet owners; a guardian arriving over sync has no distinct path here.
...
--local members
-- The frame this handler is attached to is the file-local `event`, so naming the
-- handler's own arguments `self`/`event` shadowed it. None of the three is read.
	...
	if data.Pet then
		for i = 1, #data.Pet do
			...
			if Recount:GetLazySyncTouched(myname, petname) then
				-- pet-prefixed: the player's own six totals are still in scope here
				-- and were named identically, so the two sets of numbers were told
				-- apart only by nesting.
				local petDamage = Recount:GetLazySyncAmount(myname, petname, "Damage") or 0
				local petDamageTaken = Recount:GetLazySyncAmount(myname, petname, "DamageTaken") or 0
				local petHealing = Recount:GetLazySyncAmount(myname, petname, "Healing") or 0
				local petHealingTaken = Recount:GetLazySyncAmount(myname, petname, "HealingTaken") or 0
				local petOverhealing = Recount:GetLazySyncAmount(myname, petname, "Overhealing") or 0
				local petActiveTime = Recount:GetLazySyncAmount(myname, petname, "ActiveTime") or 0
				if petDamage + petDamageTaken + petHealing + petOverhealing + petHealingTaken ~= 0 then
					tinsert(serialpetdata, Recount:Serialize("PS", myname, petname, petDamage, petDamageTaken, petHealing, petOverhealing, petHealingTaken, petActiveTime))
...
function Recount:BroadcastLazySync()
	...
	if Recount.SyncDebug then
		...
		local selfCombatant = dbCombatants[name]
		if selfCombatant and selfCombatant.lazysync or Recount.SyncDebug then
			-- Sync self
			if serialdata then
		...
		for i = 1, GetNumRaidMembers(), 1 do
			if UnitExists("raid"..i) then
				...
				if Recount.VerNum and Recount.VerNum[name] and (not realm or realm == "") then -- Elsia: Only sync if we have a valid version, and on same realm
					...
					local peer = dbCombatants[name]
					if peer and peer.lazysync and name ~= Recount.PlayerName and UnitIsConnected(name) then -- We don't sync with self
						-- Sync self
						if serialdata then
							Recount:SendCommMessage("RECOUNT", serialdata, "WHISPER", name)
						end
						...
						-- Sync pets
						for p = 1, #serialpetdata do
							Recount:SendCommMessage("RECOUNT", serialpetdata[p], "WHISPER", name)
						end

						-- Sync bosses
						for b = 1, #serialbossdata do
							Recount:SendCommMessage("RECOUNT", serialbossdata[b], "WHISPER", name)
						end
						peer.lazysync = nil
					end
				end
...
function Recount:OnCommReceive(_prefix, Msgs, distribution, target)
	-- Ambiguate's result is deliberately discarded: `target` is not read anywhere
	-- below, and the call was left in place only because it looked load-bearing.
	Ambiguate(target, "none")
	...
	elseif distribution == "RAID" or distribution == "PARTY" or distribution == "INSTANCE_CHAT" then
		local worked, cmd, owner, pet = Recount:Deserialize(Msgs)
		if worked == true then
...
function Recount:FlagSync()
	-- Only flags group members Recount already has data for. Unlike CheckVisible
	-- this does not create the combatant: someone with no recorded data has
	-- nothing to sync, and creating a record here would put an empty bar on the
	-- meter for every group member.
	if IsInRaid() and GetNumRaidMembers() > 0 then
		for i = 1, GetNumRaidMembers(), 1 do
			if UnitExists("raid"..i) then
...
function Recount:ResetLazySyncData(name)
	...
	if Recount.VerNum[name] and Recount.VerNum[name] >= Recount.MinimumV and Recount.MySyncPartners and Recount.MySyncPartners[name] then
		for _, v in pairs(Recount.MySyncPartners[name]) do
...
-- `stat` throughout, not `type`. These four functions took a parameter named
-- `type`, which shadowed the Lua `type` function cached at the top of this file.
-- No body called `type()`, so nothing was broken -- but any future one would have
-- called a string instead, and the failure would have been baffling. The value is
-- a sync stat name ("Damage", "HealingTaken", ...), which is what it is now named.
function Recount:SetLazySyncAmount(name, target, stat, amount)
	if not Recount.VerNum then
		return
	end
	...
	if Recount.VerNum[name] and Recount.VerNum[name] >= Recount.MinimumV then
		...
		Recount.MySyncPartners[name][target][stat] = amount
	end
end

function Recount:GetLazySyncAmount(name, target, stat)
	if Recount.VerNum and Recount.MySyncPartners and Recount.VerNum[name] and Recount.VerNum[name] >= MinimumV then
		return Recount.MySyncPartners and Recount.MySyncPartners[name] and Recount.MySyncPartners[name][target] and Recount.MySyncPartners[name][target][stat]
	end
end
...
function Recount:AddOwnerPetLazySyncAmount(who, stat, amount)
	...
	if who.Name == Recount.PlayerName or Recount.MySyncPartners and Recount.MySyncPartners[who.Name] then
		...
		Recount:AddLazySyncAmount(who.Name, who.Name, stat, amount)
	elseif who.Owner == Recount.PlayerName or Recount.MySyncPartners and Recount.MySyncPartners[who.Owner] then
		...
		Recount:AddLazySyncAmount(who.Owner, who.Name.." <"..who.Owner..">", stat, amount)
	elseif Recount.MySyncPartners and (who.type == "Boss" or (who.level and who.level > UnitLevel("player") + 2)) then
		...
		for k in pairs(Recount.MySyncPartners) do
			Recount:AddLazySyncAmount(k, who.Name, stat, amount) -- Recount.PlayerName
		end
	end
end

function Recount:AddLazySyncAmount(name, target, stat, amount)
	--Recount:DPrint(name.." "..target.." "..stat.." "..amount)
	Recount.MySyncPartners = Recount.MySyncPartners or { }
	Recount.MySyncPartners[name] = Recount.MySyncPartners[name] or { }
	Recount.MySyncPartners[name][target] = Recount.MySyncPartners[name][target] or { }
	local who = Recount.MySyncPartners[name][target]
	who.Touched = true
	who[stat] = who[stat] or 0
	who[stat] = who[stat] + amount
end
