...
function me:SendReport()
	local Num, Loc1, Loc2
	-- BNGetFriendInfo returns presenceName, battleTag, isBattleTagPresence,
	-- toonName, toonID and client before isOnline; only isOnline is read here, so
	-- the six in front go to a throwaway. A leading-underscore NAME is not enough
	-- for an assigned local -- luacheck still reports it as never accessed -- so
	-- this is the bare `_` it does treat as intentional.
	local _
	local isOnline, totalBNet
	...
	if Loc1 == "REALID" then
		...
		if (BNGetSelectedFriend() > 0) and (totalBNet > 0) then
			Loc2, _, _, _, _, _, _, isOnline = BNGetFriendInfo(BNGetSelectedFriend())
