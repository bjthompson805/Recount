...
-- +2000 over stock. This is the shared copy (github.com/Pimptasty/LibGraph), merged from
-- the FastGuildInvite and Recount forks; it adds graph.LabelHook so a consumer can localise
-- LibGraph's internal axis labels (digits + font), plus X-axis bottom labels behind
-- graph.XLabelsEnabled, and it carries the Retail 12.0 IsMouseOver fix and the DrawHLine
-- SetTexCoord fix. The bump is what makes THIS copy the one LibStub hands out, whatever else
-- is installed -- every consumer embeds its own copy and LibStub keeps whichever registers
-- the highest minor.
--
-- THE BAND IS +2000 AND NOT +1000, AND THAT IS NOT COSMETIC. LibStub's NewLibrary refuses an
-- EQUAL minor (`if oldminor and oldminor >= minor then return nil end`), so a tie is decided
-- by load order and the loser bails silently at `if not lib then return end`. Measured across
-- this install on 2026-08-25:
--
--   Details          90000 +    62   = 90062   stock r62,  has the MouseIsOver crash
--   Recount          90000 +    68   = 90068   stock r68,  has the MouseIsOver crash
--   FastGuildInvite  90000 + 1000+68 = 91068   private fork, has the MouseIsOver crash AND
--                                              the flipped DrawHLine SetTexCoord
--
-- At +1000 this file tied FastGuildInvite's fork exactly. `fastguildinvite` sorts early in
-- load order, so on any install carrying both, the CRASHING copy won and this one never
-- registered -- the precise outcome the bump exists to prevent. The bands are therefore:
-- +0 stock, +1000 a private per-addon fork, +2000 this shared copy, which supersedes both.
local minor = 90000 + 2000 + tonumber(("$Revision: 68 $"):match("(%d+)"))
...
-- THE CALLER OWNS THE INVARIANT, AND IT IS NOT OBVIOUS: STOP FEEDING A GRAPH YOU HAVE HIDDEN.
--
-- This appends one table per sample and bounds NOTHING. The only code that ever removes an entry
-- from self.Data is the prune inside OnUpdateGraphRealtime -- which is the graph's OnUpdate
-- handler, and the client does not run OnUpdate on a hidden frame. So a feed that keeps calling
-- this while the graph is hidden allocates forever, and the graph that would prune is the graph
-- that is not ticking. Unhide it, or stop the feed, or call ResetData.
--
-- Recount does this correctly and is worth copying: it unregisters its tracking callback when a
-- realtime window closes, so the feed stops with the window.
--
-- NOT bounded here on purpose. A cap would silently change what every existing consumer's graph
-- draws, and the +2000 minor band means they cannot decline that by shipping their own copy -- a
-- fork that wins every LibStub race owes the field bug-for-bug compatibility on everything except
-- the bug it forked to fix. Pinned by a spec in Tests/realtime_spec.lua and filed in
-- docs/AUDIT.md, addendum 4, where the alternative remedies are argued out.
...
-- AUDIT.md finding 2. A series is stored BY REFERENCE and never validated, so a caller that builds
-- its points with explicit indices, or from a filtered source that skips entries, hands us a table
-- with a hole. Neither render behaviour is a diagnosis: the old pairs() walk drew every point
-- chained in arbitrary order (a visible scribble), and the ipairs() walk draws a clean, plausible,
-- SHORTER line -- a graph that looks correct and is missing data, which is the worse failure.
--
-- This is the only place that can tell a hole from a short series, because by the time
-- RefreshLineGraph sees the table there is no caller left to name. It WARNS rather than refuses:
-- Details and Recount are live consumers and erroring on data they have always been allowed to
-- pass would break them for a defect that is theirs to fix, not ours to punish.
-- ONCE PER GRAPH PER ENTRY POINT, and never re-armed. AUDIT.md finding 4: the first version fired
-- on every call, and AddDataSeries is half the reset-then-re-add refresh idiom -- it is published
-- next to ResetData on both graph types that have it, so a meter redrawing a chart on a timer calls
-- this once per refresh, per series, for the life of the window. An unbounded per-call warning on a
-- refresh path is a different thing from a warning: one chat line per second, naming a function the
-- player did not call, about a table they cannot see. The rational response is to mute the addon,
-- and then the diagnostic has destroyed itself.
--
-- Deliberately NOT cleared in ResetData. Reset-then-re-add IS the refresh cycle, so clearing there
-- would re-arm it every redraw and reproduce the spam it exists to prevent. The first warning is
-- the one that gets read.
--
-- `points` is THE ARRAY THAT WILL ACTUALLY BE WALKED, which is not always the caller's argument.
-- On the two-array (n2) form below the walked array is points[1], and the series handed on is a
-- freshly built table -- dense by construction, always. Checking that rebuilt table instead is
-- blind by definition, and it was, on the one form every real consumer uses: Recount passes n2 at
-- all four of its call sites (GUI_Graph.lua:497,571 and GUI_CompareGraph.lua:313,403).
local function WarnIfSparse(graph, points, where, what)
	-- Not a table: the callers below error on it a line later exactly as they always did. Warning
	-- first would only replace that error with a worse-located one.
	...
	-- `#points`, not table.getn: identical on a sequence in Lua 5.1, and getn is deprecated enough
	-- that luacheck's lua51 std does not carry it.
	local length = #points
	if counted == length then
		return
	end

	graph.SparseWarned = graph.SparseWarned or {}
	if graph.SparseWarned[where] then
		return
	end
	graph.SparseWarned[where] = true

	if DEFAULT_CHAT_FRAME then
		DEFAULT_CHAT_FRAME:AddMessage(("LibGraph-2.0: %s was given %s with %d entries but a length "
			.. "of %d -- it has a hole, and the graph will stop drawing at it. "
			.. "Reported once per graph.")
			:format(where, what, counted, length))
	end
end
...
function GraphFunctions:AddDataSeries(points, color, n2, linetexture)
	...
	if n2 or (table.getn(points) == 2 and table.getn(points[1]) ~= 2) then
		-- Check BEFORE the rebuild, and check points[1] -- the ipairs walk stops at a hole there,
		-- so the truncation happens on this line and `data` comes out dense with the tail missing.
		WarnIfSparse(self, points[1], "AddDataSeries", "an X-value array")
		data = {}
		for k, v in ipairs(points[1]) do
			tinsert(data, {v, points[2][k]})
		end
	else
		WarnIfSparse(self, points, "AddDataSeries", "a points table")
	end
...
function GraphFunctions:AddFilledDataSeries(points, color, n2)
	...
	if n2 or (table.getn(points) == 2 and table.getn(points[1]) ~= 2) then
		WarnIfSparse(self, points[1], "AddFilledDataSeries", "an X-value array")
		data = {}
		for k, v in ipairs(points[1]) do
			tinsert(data, {v, points[2][k]})
		end
	else
		WarnIfSparse(self, points, "AddFilledDataSeries", "a points table")
	end
...
function GraphFunctions:FindFontString()
	-- Same guard HideFontStrings has, and for the same reason: no constructor creates
	-- self.FontStrings, so on a graph that has not drawn gridlines yet this table is nil and
	-- the pairs() below raises "bad argument #1 to 'pairs' (table expected, got nil)".
	-- CreateGridlines only gets away with it because it calls HideFontStrings first; a
	-- consumer calling this public method directly on a fresh graph does not.
	if not self.FontStrings then
		self.FontStrings = {}
	end
...
function GraphFunctions:AddPie(Percent, Color)
	...
	local LastPiece = 0
	-- ipairs, not pairs: CurPiece HALVES on every step, so this walk is only correct largest-piece
	-- first and the table's order is load-bearing. `pairs` promises no order at all (Lua 5.1
	-- manual, section 5.5) -- it happens to walk an array part in order, which is why this has
	-- worked, not a guarantee. Same reasoning as the reverse-loop note in OnUpdateGraphRealtime.
	for k, v in ipairs(PiePieces) do
		if (Percent + 0.1) > CurPiece then
			...
			-- #PiePieces, not a literal 7. AUDIT.md finding 1: the 7 WAS the table's length
			-- written out, and it survives the pairs->ipairs fix untouched because ipairs
			-- yields the same integer keys. Add a "1-256" slice and this fires one iteration
			-- early, setting LastPiece before the smallest piece and handing DrawLinePie an
			-- angle short by the 0.09 fudge; remove a slice and it never fires at all, so the
			-- boundary line is drawn slightly short on every full pie. Neither case errors and
			-- neither shows up in a percentage -- the arithmetic stays self-consistent and only
...
function GraphFunctions:CompletePie(Color)
	...
	Percent = Percent + self.Remaining
	if PiePercent ~= 0 then
		-- ipairs; see the note in AddPie. Largest piece first is a requirement, not a convenience.
		for k, v in ipairs(PiePieces) do
			if (Percent + 0.1) > CurPiece then
				local t = self:FindTexture()
				t:SetTexture(TextureDirectory..v)
...
function GraphFunctions:PieChart_OnUpdate()
	-- self:IsMouseOver(), not the MouseIsOver global. Retail 12.0 no longer
	-- defines that global and this file cached it as an upvalue at load, so it
	-- was nil for the life of the session: 148 identical errors in one sitting,
	-- reported 2026-08-25. The global was only ever a one-line forwarder to this
	-- widget method (Blizzard_UIParent/Shared/UIParent.lua:530), and the method
	-- is a SimpleScriptRegion member listed in Classic Era's own API docs too,
	-- so calling it directly is correct on every flavour and needs no guard.
	if (self:IsMouseOver()) then
		local sX, sY = self:GetCenter()
		-- GetEffectiveScale, NOT GetScale: the two differ the moment any ancestor is scaled, which
		-- is the normal case for a graph parked inside a scaled window. Pinned by two specs that
		-- put the scale on an ANCESTOR and supply it to the cursor as a literal -- swapping this
		-- for GetScale turns both red while every other assertion in that file stays green.
...
-- MIND THE SIGN -- IT IS NOT THE SAME ON ALL FOUR EDGES, and this comment is the only place a
-- caller reading the code will find that out.
--
-- `size` replaces the automatic 10% margin for one edge, and RefreshLineGraph applies EVERY edge as
-- `extreme + size` (:1900-1930). But the default it replaces is not symmetric -- `XMin = MinX -
-- XBorder` and `YMin = MinY - YBorder` SUBTRACT, while XMax and YMax add. So:
--
--     TOP, RIGHT     positive `size` widens the axis outward. "Border" means what you expect.
--     BOTTOM, LEFT   positive `size` moves the axis INWARD and CROPS the data.
--                    Pass a NEGATIVE number on these two to add room.
--
-- This is upstream's behaviour (identical in the stock r68 copy) and it is kept deliberately rather
-- than corrected: the +2000 minor band means this copy wins LibStub's race in every install, for
-- every addon, so a stock consumer calling SetBorderSize("LEFT", 10) cannot decline a semantic
-- change by shipping their own copy -- ours outranks theirs by construction. A fork that wins every
-- race owes the field bug-for-bug compatibility on everything except the bug it forked to fix.
-- Full reasoning in docs/AUDIT.md, addendum 3 and review round 6 (finding 6).
function GraphFunctions:SetBorderSize(border, size)
	...
	-- The sign half is added to upstream's wording on purpose: this assert is the ONE diagnostic a
	-- caller who got it wrong actually sees, and naming only the four edges teaches them the half
	-- they already knew. AUDIT.md finding 6.
	return assert (false, "Usage: GraphObject:SetBorderSize (LEFT RIGHT TOP BOTTOM, SIZE) -- "
		.. "size is ADDED to the data extreme on every edge, so LEFT and BOTTOM need a NEGATIVE "
		.. "size to add room rather than crop")
end
			...
			if i ~= 0 or not self.YAxisDrawn then
				...
				if ((i ~= UpperYGridLine) or (TopSpace > 12)) and (NoSecondary or math_fmod(i, self.GridSecondaryY) == 0) then
					if self.YLabelsLeft then
						...
						if self.LabelHook then self.LabelHook(F) end
						F:Show()
					end
	...
	if self.XGridInterval then
		...
		for i = LowerXGridLine, UpperXGridLine do
			if i ~= 0 or not self.XAxisDrawn then
				...
				-- Add X-axis labels at bottom (skip the rightmost label)
				if self.XLabelsEnabled and i < UpperXGridLine and (NoSecondary or math_fmod(i, self.GridSecondaryX) == 0) then
					F = self:FindFontString()
					F:SetFontObject("GameFontHighlightSmall")
					F:SetTextColor(1, 1, 1)
					F:ClearAllPoints()
					F:SetPoint("TOP", self, "BOTTOMLEFT", XPos, -2)
					F:SetText(i * self.XGridInterval)
					if self.LabelHook then self.LabelHook(F) end
					F:Show()
				end
			end
...
function GraphFunctions:OnUpdateGraphRealtime()
	...
	if self.Mode == "SLOW" then
		...
		if self.Filter == "RECT" then
			...
			local DataValue = 1 / (2 * self.TimeRadius)
			-- REVERSE NUMERIC, not pairs(). This loop removes from the table it is walking.
			--
			-- NOT undefined behaviour, and the distinction is the whole explanation. The Lua 5.1
			-- manual says `next` is undefined only if you assign to a NON-EXISTENT field, and
			-- then: "You may however modify existing fields. In particular, you may clear
			-- existing fields." tremove() shifts existing fields down and clears the last, so the
			-- traversal was explicitly PERMITTED all along.
			--
			-- What it is instead is a deterministic race: tremove shifts t[k+1] into t[k] while
			-- the iterator increments past k, so whatever slid into the visited slot is skipped.
			-- Undefined behaviour has no signature; this does -- measured at exactly half, six
			-- stale samples in and three left, on three of four prune paths.
			--
			-- Walking backwards makes a removal only affect indices already passed. The
			-- accumulation below is a sum, so visiting in reverse changes nothing else.
		...
		elseif self.Filter == "TRI" then
			...
			-- Reverse numeric; see the note on the RECT loop above.
			for k = #self.Data, 1, -1 do
				local v = self.Data[k]
				local Temp
				if v.Time < (CurTime + self.XMin - self.TimeRadius) then
	...
	elseif self.Mode == "FAST" then
		...
		if ShiftBars > 0 and not (self.LastDataTime < (self.LastShift + self.XMin - self.TimeRadius * 2)) then
			...
			if self.Filter == "RECT" then
				--Take the convolution of the dataset on to the bars wtih a rectangular filter
				local DataValue = 1 / (2 * self.TimeRadius)
				-- Reverse numeric; see the note on the SLOW/RECT loop above.
				for k = #self.Data, 1, -1 do
					local v = self.Data[k]
	...
	elseif self.Mode == "EXP" then
		...
		if ShiftBars > 0 then
			...
			for i = RecalcBars, self.BarNum do
				...
				-- Reverse numeric; see the note on the SLOW/RECT loop above. Total is a sum, and a
				-- sample still contributes on the pass that removes it, exactly as before.
				for k = #self.Data, 1, -1 do
					local v = self.Data[k]
...
function GraphFunctions:RefreshLineGraph()
	...
	for k1, series in pairs(self.Data) do
		...
		-- ipairs, not pairs: this chains a line from each point to the NEXT one, so the sequence
		-- order is the whole meaning of the loop. `pairs` guarantees no order (Lua 5.1 manual,
		-- section 5.5); it walks an array part in order in practice, which is why this has always
		-- drawn correctly, but the code was relying on an implementation detail to draw a graph.
		-- The one real difference: ipairs stops at the first nil, so a caller passing a SPARSE
		-- points table now gets a truncated line rather than segments joined in arbitrary order.
		-- Truncated-but-coherent is the better failure.
	...
	for k1, series in pairs(self.FilledData) do
		...
		-- ipairs; same chaining requirement as the line series above.
		for k2, point in ipairs(series.Points) do
			if LastPoint then
				local TPoint = {x = point[1]; y = point[2]}
...
function lib:DrawHLine(C, sx, ex, y, w, color, layer)
	...
	-- Set texture coordinates and anchors
	T:ClearAllPoints()
	-- 4-param form, which is the exact equivalent of the 8-param
	-- SetTexCoord(0,0, 0,1, 1,0, 1,1) stock uses here: UL(0,0) LL(0,1) UR(1,0)
	-- LR(1,1) is the identity mapping, i.e. left=0 right=1 bottom=0 top=1
	-- (arg order per SimpleTextureBaseAPIDocumentation.lua:395).
	-- The FGI copy had (0, 1, 1, 0), which swaps the last two and draws sline
	-- vertically flipped; harmless only if the texture is symmetric, and nothing
	-- in FGI wanted a flip -- the intent was just to drop the 8-param form.
...
function lib:DrawBar(C, sx, sy, ex, ey, color, level)
	...
	-- `self` MEANS TWO DIFFERENT THINGS IN THIS FUNCTION, so neither branch below may use it.
	-- DrawBar is reached two ways: internally as `self:DrawBar(self, ...)` from RefreshLineGraph,
	-- where `self` is the GRAPH; and publicly as `lib:DrawBar(frame, ...)`, which is the documented
	-- primitive and how Details draws, where `self` is the LIBRARY TABLE shared by every graph in
	-- the install. `C` is the frame being drawn into on both paths, so it is the only safe subject.
	-- AUDIT.md addendum 5.
	if level then
		if type(C.GraphLib_Frames[level]) == "nil" then
			...
			if C.TextFrame and C.TextFrame:GetFrameLevel() <= newLevel then
				...
				-- Was `self.NeedsUpdate`, which asked the GRAPH for a redraw on the internal path
				-- and wrote a dead field onto the shared library table on the public one.
				C.NeedsUpdate = true
			end
		end
		...
		-- REQUIRED, because the pool does not reset the parent. HideBars returns a bar with only
		-- Hide() (:2386-2394), so a bar that was last drawn at a level is still parented to that
		-- level frame when it comes back out. Without this, an unlevelled DrawBar reusing it would
		-- silently draw at whatever level the previous caller happened to use -- a wrong picture
		-- that depends on pool order, which is not a behaviour anyone can be depending on.
		Bar:SetParent(C)
		Tri:SetParent(C)
	end
...
-- ---------------------------------------------------------------------------
-- VersionCheck-1.0 -- ONLY when this copy is the STANDALONE ADDON, never when embedded.
--
-- THE GATE IS THE WHOLE POINT AND IT IS NOT DEFENSIVE PROGRAMMING. This exact file ships inside
-- other addons' Libs folders (Details, Recount, FastGuildInvite all embed a copy). An embedded copy
-- is loaded by ITS HOST's TOC, so:
--
--   * there is no addon called "LibGraph-2.0" for the client to have metadata about, and
--   * an embedded copy is not separately updatable -- telling a player their LibGraph is out of
--     date when it arrived inside Recount is advice they cannot act on.
--
-- GetAddOnMetadata(major, "Version") is exactly the right test: it is non-nil only when a real
-- addon FOLDER named LibGraph-2.0 is installed and loaded, which is the standalone case and the
-- only case where a version check means anything.
--
-- The `@` guard catches an UNPACKAGED working copy. The TOC ships `## Version: Recount-v1.18.8`
-- and only the BigWigs packager substitutes it, so a developer running from a git checkout would
-- otherwise broadcast the literal placeholder to their guild as a version string.
--
-- Enable() is the entire integration surface. Do NOT add a `VC.RegisterCheck` branch -- that name
-- has never existed, and VersionCheck's own README records two shipped addons that wrote
-- `if VC.RegisterCheck then ... elseif VC.Enable then ...`, which works only because the first
-- branch is dead. If it ever becomes live, those addons silently stop registering.
do
	-- Read through _G: WoW 11.x moved GetAddOnMetadata into C_AddOns and dropped the bare global,
	-- and Classic Era took the same change, so both spellings have to be reachable without either
	-- one being a declared global here.
	local C_AddOnsNS = _G.C_AddOns
	local GetAddOnMetadataFn = (C_AddOnsNS and C_AddOnsNS.GetAddOnMetadata) or _G.GetAddOnMetadata
	local version = GetAddOnMetadataFn and GetAddOnMetadataFn(major, "Version")

	if version and version ~= "" and not version:find("@", 1, true) then
