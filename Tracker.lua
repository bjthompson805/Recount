...
local revision = tonumber(string.sub("$Revision: 1606 $", 12, -3))
...
--
-- TRIMMED TO WHAT RECOUNT ACTUALLY READS. The upstream table declares the
-- combat log's whole flag layout; carrying the unused half here only produced
-- unused-variable warnings, and the full layout is documented in Blizzard's own
-- source rather than needing a second copy in this file. Add a constant back
-- when a use for it appears.
...
-- Prayer of Mending source-attribution fix.
-- The combat log reports the buffed unit as the source of the PoM heal
-- (both srcGUID and dstGUID equal the target). Track who originally cast
-- the buff so we can credit the real healer in SpellHeal. The buff bounces
-- between targets via fresh SPELL_AURA_APPLIED events, so the table self-
-- updates as PoM moves around.
local PoMAuraSpellId = {
	[41635]	= true, -- Prayer of Mending (buff)
}
local PoMHealSpellId = {
	[33110]	= true, -- Prayer of Mending (heal proc)
}
local PoMOrigCaster = { }
...
local AbsorbSpellDuration = {
	...
	[43019]		= 60,
	[43020]		= 60, -- Rank 9
	--[1463]		= 8, -- Incanter's Ward (Mage) (may have unverified aura trigger), MOP (duplicate key, overridden above)
	...
	[27128]		= 30,
	[43010]		= 30, -- Rank 7
	--[1463]		= 8, -- Incanter's Ward (Mage) (may have unverified aura trigger), MOP (duplicate key, see Mana Shield above)
	-- Monk
	...
	[47890]		= 30,
	[47891]		= 30, -- Rank 6
	--[6229]		= 30, -- Twilight Ward (partially confirmed), MOP (duplicate key, same value as Shadow Ward above)
	...
	--[31000]		= 300, -- duplicate key removed
...
--[=[function Recount:AddGuessedAbsorbData(source, victim, ability, element, hittype, damage, resist, srcGUID, srcFlags, dstGUID, dstFlags, spellId, blocked, absorbed, timestamp)
	...
	if not minsrc then
		...
		if not Recount.db2.combatants[source] then
			...
			-- Stamped for the same reason as AddAbsorbCredit; see there.
			-- THIS WHOLE FUNCTION IS DISABLED: it sits inside a long-bracket
			-- comment opened at the AddGuessedAbsorbData line above and closed
			-- after the final end below, so nothing here executes. An earlier
			-- note at this line said "no spec reaches this block", which was true
			-- and badly framed -- it read as live code lacking coverage, and it
			-- is not live code at all. Kept rather than deleted because it
			-- records how absorbs were guessed before SPELL_ABSORBED existed.
...
-- EVERY HANDLER BELOW IS DISPATCHED POSITIONALLY out of EventParse, so each one
-- declares the client's full argument list whether or not it reads each name.
-- Unread positions carry a LEADING UNDERSCORE rather than being dropped: the
-- name still records what that slot holds, dropping one would silently shift
-- every argument after it, and the prefix is what tells luacheck the omission is
-- deliberate. Do not delete an underscored parameter; rename it if it gains a use.
function Recount:SpellBuildingHeal(_timestamp, _eventtype, _srcGUID, _srcName, _srcFlags, _dstGUID, _dstName, _dstFlags, _spellId, _spellName, _spellSchool, _amount, _overheal, _critical)
	-- Ignoring these for now
end

function Recount:SpellDamage(_timestamp, eventtype, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, spellId, spellName, spellSchool, amount, _overkill, school, resisted, blocked, absorbed, critical, glancing, crushing, _isOffHand)
	-- Prismatic Crystal
...
function Recount:EnvironmentalDamage(_timestamp, _eventtype, srcGUID, _srcName, _srcFlags, dstGUID, dstName, dstFlags, enviromentalType, amount, _overkill, school, resisted, blocked, absorbed, critical, glancing, crushing)
	local HitType = "Hit" -- Elsia: Do NOT localize this, it breaks functionality!!! If you need this localized contact me on WowAce or Curse.
	if critical then
		HitType = "Crit" -- Elsia: Do NOT localize this, it breaks functionality!!! If you need this localized contact me on WowAce or Curse.
	end
...
function Recount:SwingMissed(_timestamp, _eventtype, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, missType, _isOffHand, amountMissed)
	local blocked
	local absorbed
...
function Recount:SpellMissed(_timestamp, eventtype, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, spellId, spellName, spellSchool, missType, _isOffHand, amountMissed)
	local blocked
	local absorbed
...
function Recount:SpellHeal(_timestamp, eventtype, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, spellId, spellName, _spellSchool, amount, overheal, absorbed, critical)
	-- Prayer of Mending: the combat log lists the buffed unit as the source,
	-- so substitute the original caster recorded at aura application time.
	if PoMHealSpellId[spellId] then
		local orig = PoMOrigCaster[srcGUID]
		if orig then
			srcGUID, srcName, srcFlags = orig[1], orig[2], orig[3]
		end
	end
...
function Recount:SpellExtraAttacks(_timestamp, _eventtype, _srcGUID, srcName, _srcFlags, _dstGUID, _dstName, _dstFlags, _spellId, spellName, _spellSchool, amount)
	...
	-- A second proc while one is still pending is ignored: the first one's ability
	-- is still waiting to be matched to its damage event, and overwriting it would
	-- attribute that damage to the wrong proc.
	if not extraattacks[srcName] then
...
function Recount:SpellDrain(_timestamp, _eventtype, _srcGUID, _srcName, _srcFlags, _dstGUID, _dstName, _dstFlags, _spellId, _spellName, _spellSchool, _amount, _powerType, _extraAmount)
	-- Currently unused.
end

function Recount:AddAbsorbCredit(source, victim, spellName, _spellId, absorbed)
	-- An absorb whose caster is not a tracked combatant is dropped: the shield's
	-- owner is who the credit belongs to, and there is nobody else to give it to.
	if source and Recount.db2.combatants[source] then
		local sourceData = Recount.db2.combatants[source]
		-- Stamp both ends, as AddHealData does at :2239 and :2257. SetActive only
		-- writes LastActive, whose one reader is the idle sweep at
		-- Recount.lua:1607 -- a recorder that credits a combatant without stamping
		-- it can watch that record be deleted out from under it 30 seconds later.
		Recount:SetActive(sourceData)
		Recount:AddAmount(sourceData, "Absorbs", absorbed)
		Recount:AddTableDataStats(sourceData, "Absorbed", spellName, victim, absorbed)
...
local function IgnoreAurasUpdate(self, elapsed)
	...
	if self.time > 0.2 then
		...
		for k, v in pairs(IgnoreAuras) do
			-- Clear the TABLE entry, not the loop variable. Assigning to `v`
			-- expired nothing, so IgnoreAuras grew without bound and `num`
			-- counted every aura ever added -- which also meant this OnUpdate
			-- handler could never reach zero and unregister itself.
...
function Recount:SpellAuraApplied(_timestamp, _eventtype, srcGUID, srcName, srcFlags, dstGUID, dstName, _dstFlags, spellId, spellName, _spellSchool, _auraType, _amount)
	-- Spirit Shift
	...
	if PoMAuraSpellId[spellId] and dstGUID then
		PoMOrigCaster[dstGUID] = { srcGUID, srcName, srcFlags }
	end

	if AbsorbSpellDuration[spellId] then
		if Recount.db2.combatants[srcName] then
			local sourceData = Recount.db2.combatants[srcName]
			-- Stamped for the same reason as AddAbsorbCredit at :727. This one
			-- matters MORE than the scalar recorders, not less: AddTableDataSum
			-- records onto a combatant whose own type is filtered out whenever
			-- the secondary is tracked (:1623-1632), so a shield caster can hold
			-- a ShieldedWho record while failing the idle sweep's filter test at
			-- Recount.lua:1607 -- the one combination AddAmount cannot produce.
			Recount:SetActive(sourceData)
...
function Recount:SpellAuraRefresh(_timestamp, _eventtype, srcGUID, srcName, srcFlags, dstGUID, dstName, _dstFlags, spellId, spellName, _spellSchool, _auraType, _amount)
	if PoMAuraSpellId[spellId] and dstGUID then
		PoMOrigCaster[dstGUID] = { srcGUID, srcName, srcFlags }
	end
	...
	if AbsorbSpellDuration[spellId] then
		if Recount.db2.combatants[srcName] then
			local sourceData = Recount.db2.combatants[srcName]
			-- Stamped for the same reason as the identical block in
			-- SpellAuraApplied at :791; see the comment there.
			Recount:SetActive(sourceData)
			Recount:AddTableDataSum(sourceData, "ShieldedWho", dstName, spellName, 1)
		end
	end
...
function Recount:SpellAuraRemoved(_timestamp, _eventtype, srcGUID, _srcName, _srcFlags, dstGUID, _dstName, _dstFlags, spellId, _spellName, _spellSchool, _auraType, _amount)
	if spellId == 184293 then
		IgnoreAuras[srcGUID] = nil
	end

	if PoMAuraSpellId[spellId] and dstGUID then
		PoMOrigCaster[dstGUID] = nil
	end
...
function Recount:SpellAuraAppliedRemovedDose(_timestamp, _eventtype, _srcGUID, _srcName, _srcFlags, _dstGUID, _dstName, _dstFlags, _spellId, _spellName, _spellSchool, _auraType, _amount)
	-- Not sure yet how to handle this
end

function Recount:SpellCastStartSuccess(_timestamp, eventtype, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, spellId, _spellName, _spellSchool)
...
function Recount:SpellCastFailed(_timestamp, _eventtype, _srcGUID, _srcName, _srcFlags, _dstGUID, _dstName, _dstFlags, _spellId, _spellName, _spellSchool, _failedType)
	-- Not sure yet how to handle this, are these interrupts?
end

function Recount:EnchantAppliedRemoved(_timestamp, _eventtype, _srcGUID, _srcName, _srcFlags, _dstGUID, _dstName, _dstFlags, _spellName, _itemId, _itemName)
	-- Not sure yet how to handle this,
end

function Recount:PartyKill(_timestamp, _eventtype, _srcGUID, _srcName, _srcFlags, _dstGUID, _dstName, _dstFlags)
	--Recount:AddDeathData(srcName , dstName, nil, srcGUID, srcFlags, dstGUID, dstFlags, nil)
	-- Could be killing blow tracker
end

function Recount:UnitDied(_timestamp, _eventtype, srcGUID, _srcName, srcFlags, dstGUID, dstName, dstFlags)
	Recount:AddDeathData(nil , dstName, nil, srcGUID, srcFlags, dstGUID, dstFlags, nil)
end

function Recount:SpellSummon(_timestamp, _eventtype, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags)
	Recount:AddPetCombatant(dstGUID, dstName, dstFlags, srcGUID, srcName, srcFlags)
end

function Recount:SpellCreate(_timestamp, _eventtype, _srcGUID, _srcName, _srcFlags, _dstGUID, _dstName, _dstFlags, _spellId, _spellName, _spellSchool)
	-- Elsia: We do nothing for these yet.
end

-- SPELL_ABSORBED arrives in two shapes: a swing absorb has no source spell, so
-- the caster block starts at argument 9, while a spell absorb inserts the source
-- spell's id/name/school first and pushes it to argument 12. Argument 9 being a
-- number is what tells them apart.
--
-- The two layouts, written out once here because they are the only place the
-- positions are recorded. The spell form's three extra fields sit between the
-- destination block and the caster block, so everything from argument 9 onward
-- shifts by exactly three:
--
--   1 timestamp   2 eventtype   3 srcGUID   4 srcName   5 srcFlags
--   6 dstGUID     7 dstName     8 dstFlags
--     -- spell form only: 9 srcSpellId  10 srcSpellName  11 srcSpellSchool
--   9(+3) casterGUID  10(+3) casterName  11(+3) casterFlags  12(+3) casterRaidFlags
--   13(+3) spellId    14(+3) spellName   15(+3) spellSchool  16(+3) absorbed
--
-- Read by index rather than unpacked into a full name list, so no name exists
-- that nothing uses, and the one thing that genuinely differs between the two
-- forms is a single offset rather than a duplicated function body.
function Recount:SpellAbsorbed(...)
	local arg9 = select(9, ...)
	local off = type(arg9) == "number" and 3 or 0

	local dstName = select(7, ...)
	local casterGUID, casterName, casterFlags = select(9 + off, ...)
	local spellId, spellName = select(13 + off, ...)
	local absorbed = select(16 + off, ...)
	...
	local _, casterowner = Recount:DetectPet(casterName, casterGUID, casterFlags)
	if casterowner then
...
function Recount:CheckRetentionFromFlags(nameFlags, _name, nameGUID)
...
function Recount:CombatLogEvent(timestamp, eventtype, _hideCaster, srcGUID, srcName, srcFlags, _srcRaidFlags, dstGUID, dstName, dstFlags, _dstRaidFlags, ...)
...
function Recount:AddTimeEvent(who, onWho, ability, friendly, _pet)
	if not who then
		return
	end
...
function Recount:AddCurrentEvent(who, eventType, incoming, _number, event)
	if not who then
		return
	end
...
function Recount:DetectPet(name, nGUID, nFlags)
	...
	-- Strip a "<Owner>" suffix the name may already carry, so lookups run on the
	-- bare pet name. Only the first capture is taken: the owner named in the
	-- string is deliberately discarded and re-derived from the GUID below, which
	-- is authoritative where the formatted string is only as good as whoever
	-- built it.
	...
	if nFlags and bit_band(nFlags, COMBATLOG_OBJECT_TYPE_PET + COMBATLOG_OBJECT_TYPE_GUARDIAN) ~= 0 then
		...
		elseif bit_band(nFlags, COMBATLOG_OBJECT_AFFILIATION_PARTY + COMBATLOG_OBJECT_AFFILIATION_RAID) ~= 0 then
			...
			-- GetGuardianOwnerByGUID answers with a formatted "Guardian <Owner>"
			-- string, so only the owner half is wanted here; the pet-name half
			-- was being captured into a variable nothing read.
			local guardian = Recount:GetGuardianOwnerByGUID(nGUID)
			if guardian then
				owner = guardian:match(".- <(.*)>")
				return name, owner, ownerID
			end
		end
	end
...
-- "Who are we fighting" -- the highest-level hostile the group is currently
-- trading blows with, which drives the fight name.
--
-- THE FIRST TWO PARAMETERS ARE ROLES, NOT COMBAT-LOG POSITIONS, and they used to
-- be named `srcFlags`/`dstFlags` after the positions instead. Both call sites in
-- AddDamageData pass them in OPPOSITE order and both are correct: the enemy is
-- the combat log's destination when a group member is attacking and its source
-- when a group member is being hit, so each block supplies whichever end the
-- enemy is on. Named for the role so the reversal reads as deliberate.
--
-- This is the same shape as the AddGain defect (docs/AUDIT.md finding 7) and it
-- is NOT broken, for one reason worth keeping: these arrive as PARAMETERS, so
-- each caller states its own orientation. AddGain read the equivalent pair from
-- globals, which cannot follow a caller's swap, and that is what made it wrong
-- for one of its two callers. Do not "simplify" these into globals.
function Recount:BossFightWho(groupSideFlags, enemySideFlags, enemyData, enemyName)
	if Recount:InGroup(groupSideFlags) and not Recount:IsFriend(enemySideFlags) then
		if not enemyData.level then
			--Recount:Print(enemyData.Name.." lacks level, please report") -- This happens for freezing traps intriguingly enough
			enemyData.level = 1
		end
		if (enemyData.level == -1) or ((Recount.FightingLevel ~= -1) and (enemyData.level > Recount.FightingLevel)) then
			Recount.FightingWho = enemyName
			Recount.FightingLevel = enemyData.level
		end
	end
end

-- Same two roles as BossFightWho above, for the case where the enemy has no
-- combatant record yet and only its GUID is available. Both its call sites also
-- pass the pair in opposite orders, for the same correct reason.
function Recount:BossFightWhoFromFlags(groupSideFlags, enemySideFlags, enemyName, enemyGUID)
	if Recount:InGroup(groupSideFlags) and not Recount:IsFriend(enemySideFlags) then
		if Recount.IsBoss(enemyGUID) then
			Recount.FightingWho = enemyName
			Recount.FightingLevel = -1
...
function Recount:AddDamageData(source, victim, ability, element, hittype, damage, resist, srcGUID, srcFlags, dstGUID, dstFlags, spellId, blocked, absorbed, isDot)
	...
	-- Credit whoever is preventing damage on the victim. Deliberately here and
	-- not in either retention block: the aura's caster is a third party to this
	-- event, so gating on the attacker's or the victim's retention would drop
	-- their contribution for reasons that have nothing to do with them. It must
	-- also run BEFORE the srcRetention block rewrites `element` to "Melee"
	-- below, because the mitigation registry matches on the damage school.
	if Recount.CreditMitigation then
		Recount:CreditMitigation(victim, ability, element, hittype, damage, dstGUID)
	end
			...
			if sourceData and Recount.db.profile.Filters.Data[sourceData.type] then
				...
				if damage then
					...
					if not FriendlyFire then
						...
						-- Friendly damage is deliberately not synced: it is not part
						-- of anyone's contribution to the fight, so it would only
						-- cost bandwidth.
...
function Recount:HandleDoubleDeath(_source, victim, skill, _srcGUID, _srcFlags, dstGUID, dstFlags, spellId)
	if dstRetention then
		-- Name and ID of pet owners
		local victimowner
		local victimownerID
...
local deathargs = {}
local timeofdeath
function Recount:AddDeathData(source, victim, skill, srcGUID, srcFlags, dstGUID, dstFlags, _spellId)
	...
	if dstRetention then
		...
		if victimData then
			...
			--Fight tracking purposes to speed up leaving combat
			victimData.LastFightIn = Recount.db2.FightNum
			-- A death is the least idle thing a combatant can do, and this block
			-- writes DeathCount, DeathLog and the current-event line to a record
			-- nothing else here stamps. LastFightIn is not a substitute: the idle
			-- sweep at Recount.lua:1607 reads LastActive and nothing else.
			Recount:SetActive(victimData)
			...
			elseif skill then
				...
				victimData.LastKilledAt = timeofdeath
			-- We don't count spirits and ghouls: a second death arriving within
			-- 2s of a double-death marker is the same death. The delay used to
			-- come from a `doubleDeathDelay` local whose only assignment was
			-- commented out at :2458, so it was always nil and this line threw
			-- "attempt to compare nil with number" for any death that had
			-- neither a source nor a skill while a marker was set. Computed
			-- inline now, with a missing DoubleDeathTime meaning "not a double
			-- death" -- the same fallback the commented-out line used (10).
			elseif not victimData.DoubleDeathSpellID
				or not victimData.DoubleDeathTime
				or (timeofdeath - victimData.DoubleDeathTime) >= 2 then
...
function Recount:HandleDeath(arg)
	-- AddDeathData packs four values into `arg`; only the first two are read here.
	-- The GUID and flags are still packed because the commented-out direct-call
	-- form at the pack site passes them, so dropping them there would change what
	-- that line has to say when it is used again.
	local victim, DeathTime = unpack(arg)
...
function Recount:SpellAuraDispelFailed(_timestamp, _eventtype, _srcGUID, _srcName, _srcFlags, _dstGUID, _dstName, _dstFlags, _spellId, _spellName, _spellSchool, _extraSpellId, _extraSpellName, _extraSpellSchool)
