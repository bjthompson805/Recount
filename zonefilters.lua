...
function Recount:SetZoneGroupFilter(instanceType, groupType)
	...
	if Recount.db.profile.ZoneFilters[instanceType] and Recount.db.profile.GroupFilters[groupType] then
		...
		-- Parenthesised deliberately: without them this reads as
		-- (HideCollect and IsShown()) or (not GlobalDataCollect), which hides
		-- the window whenever global collection is off even though the user
		-- never enabled auto-hide. Matches SetZoneFilter / SetGroupFilter.
...
function Recount:UpdateZoneGroupFilter()
	...
	if not instanceType then
		local _, scenarioType = C_Scenario.IsInScenario()
		instanceType = scenarioType or "none"
	end
