...
-- Combat-log handlers are dispatched POSITIONALLY, so this declares the client's
-- full argument list whether or not it reads each name. Unread positions carry a
...
function Recount:SpellAuraDispelledStolen(_timestamp, eventtype, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, _spellId, spellName, _spellSchool, extraSpellId, extraSpellName, _extraSpellSchool)
...
function Recount:AddDispelData(source, victim, ability, srcGUID, srcFlags, dstGUID, dstFlags, _spellId)
