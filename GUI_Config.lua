...
function Recount:ResetDefaultClassColors()
	for _, v in pairs(EditableColors.Class) do
...
function me:CreateColorRow(parent)
...
function me:CreateWindowColorSelection(parent)
	...
	i = i + 1
	for _, v in pairs(EditableColors.Window) do
...
function me:CreateWindowModuleSelection(parent)
	...
	theFrame.Threat = me:CreateSavedCheckbox("Threat", theFrame, "Modules", "Threat")
	theFrame.Threat:SetPoint("TOPLEFT", theFrame.Activity, "BOTTOMLEFT", 0, 0)
	theFrame.Threat:SetScript("OnClick", function(this)
		if this:GetChecked() then
			this:SetChecked(true)
			Recount.db.profile.Modules.Threat = true
			Recount:SetupMainWindow()
			Recount:RefreshMainWindow()
		else
			this:SetChecked(false)
			Recount.db.profile.Modules.Threat = false
			Recount:SetupMainWindow()
			Recount:RefreshMainWindow()
		end
	end)
...
function me:CreateClassColorSelection(parent)
	...
	local i = 1
	for _, v in pairs(EditableColors.Bar) do
		theFrame.Rows[i] = me:CreateColorRow(theFrame)
	...
	-- No `i = i + 1` here: the row counter is not read again after this point, so
	-- the increment only looked like it was reserving a slot. The reset button
	-- below is positioned from a fixed anchor, not from `i`.
	theFrame.ResetColButton = CreateFrame("Button", nil, theFrame, "UIPanelButtonTemplate")
	theFrame.ResetColButton:SetWidth(120)
	theFrame.ResetColButton:SetHeight(18)
	--theFrame.ResetColButton:SetPoint("TOPLEFT", theFrame, "TOPLEFT", 40, -210)
...
function me:BarTextureDropDown_Initialize()
	local BarTextures = SM:List("statusbar")
	-- Was three statements: declare `LookingFor`, then `if not LookingFor then`
	-- twice. The first test could only ever be true, because nothing had assigned
	-- it -- luacheck flags it as reading an uninitialised variable. The intent is
	-- the profile's texture with a fallback, which is what this says.
	local LookingFor = Recount.db.profile.BarTexture or "BantoBar"
...
-- `_num` stays in place: callers pass the row number positionally even though
-- this function does not use it. Dropping it shifts every argument after it.
function me:SetTestBar(_num, left, right, value, color)
...
function me:CreateBarSelection(parent)
	...
	-- The height each GetFont returns is discarded -- both texts are re-set to a
	-- fixed 14 * 0.75 -- so only the face and flags are taken. The second pair was
	-- re-declared with `local` and shadowed the first.
	local LeftFont, _, LeftFlags = row.LeftText:GetFont()
	row.LeftText:SetFont(LeftFont, 14 * 0.75, LeftFlags)
	local RightFont, _, RightFlags = row.RightText:GetFont()
	row.RightText:SetFont(RightFont, 14 * 0.75, RightFlags)
...
function me:CreateTextureSelection(parent)
	...
	theFrame.ScrollBar:SetScript("OnVerticalScroll", function(scrollFrame, offset)
		FauxScrollFrame_OnVerticalScroll(scrollFrame, offset, 12, me.RefreshStatusBars)
	end)
...
function me:CreateFontSelection(parent)
	...
	theFrame.ScrollBar:SetScript("OnVerticalScroll", function(scrollFrame, offset)
		FauxScrollFrame_OnVerticalScroll(scrollFrame, offset, 12, me.RefreshFonts)
		end)
	theFrame.ScrollBar:SetPoint("TOPLEFT", theFrame.Rows[1], "TOPLEFT")
	theFrame.ScrollBar:SetPoint("BOTTOMRIGHT", theFrame.Rows[13], "BOTTOMRIGHT", -5, 0)
...
function me:SetupWindowOptions(parent)
	...
	theFrame.ResetWinButton = CreateFrame("Button", nil, theFrame, "UIPanelButtonTemplate")
	...
	slider:SetScript("OnMouseWheel", function(bar, delta)
		bar:SetValue(bar:GetValue() + (delta * bar:GetValueStep()))
	end)
...
function me:SetupRealtimeOptions(parent)
	...
	theFrame.RDPSButton = CreateFrame("Button", nil, theFrame, "UIPanelButtonTemplate")
	...
	theFrame.RDTPSButton = CreateFrame("Button", nil, theFrame, "UIPanelButtonTemplate")
	...
	theFrame.RHPSButton = CreateFrame("Button", nil, theFrame, "UIPanelButtonTemplate")
	...
	theFrame.RHTPSButton = CreateFrame("Button", nil, theFrame, "UIPanelButtonTemplate")
	...
	theFrame.LATButton = CreateFrame("Button", nil, theFrame, "UIPanelButtonTemplate")
	...
	theFrame.UPTButton = CreateFrame("Button", nil, theFrame, "UIPanelButtonTemplate")
	...
	theFrame.DOTButton = CreateFrame("Button", nil, theFrame, "UIPanelButtonTemplate")
	...
	theFrame.BWButton = CreateFrame("Button", nil, theFrame, "UIPanelButtonTemplate")
...
function me:SetupMiscOptions(parent)
	...
	theFrame.VerChkButton = CreateFrame("Button", nil, theFrame, "UIPanelButtonTemplate")
...
function me:SetupButtonOptions(parent)
	...
	-- Reassigned rather than re-declared: `slider` is reused for each of the
	-- sliders built below and every one is finished configuring before the next.
...
function me:LoadConfig()
	...
	for k in ipairs(GroupLabels) do
